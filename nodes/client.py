...
class GenClient(GenNode):
    ...
    def local_train_cgan_step(self, model: Dict[str, torch.nn.Module], loss_func: torch.nn.Module, optimizer: Dict[str, torch.optim.Optimizer], batch_size: int, encoder: OneHotEncoder4Gan, local_rounds: int = 1, noise_dim: int = 100, num_workers: int = 0, **optimizer_settings) -> Tuple[List[float], List[float], List[float], List[float], float, float]:
        model_optimizer: Dict[str, torch.optim.Optimizer] = {}
        for type in ('d', 'g'):
            # model loading
            for layer, param in enumerate(model[type].parameters(), 1):
        ...
        d_loss, g_loss = 0., 0.
        for _ in range(local_rounds):
            for batch_idx, (real_samples, real_labels) in enumerate(dataloader, 1):
                batch_size = real_samples.shape[0]
                real_samples, real_labels = real_samples.to(self.device), real_labels.to(self.device)

                ### Train Discriminator ###
                model_optimizer['d'].zero_grad()

                fake_labels = torch.randint(0, encoder._num_class_, (batch_size, ), device=self.device)
                onehot_fake_labels_g = encoder.encode(fake_labels, type='g')
                onehot_fake_labels_d = encoder.encode(fake_labels, type='d')
                onehot_real_labels_d = encoder.encode(real_labels, type='d')

                with torch.no_grad():
                    noise_samples = self.__gen_noise__(batch_size=batch_size, noise_dim=noise_dim)
                    fake_samples: torch.Tensor = model['g'](noise_samples, onehot_fake_labels_g)

                real_samples_pred = model['d'](real_samples, onehot_real_labels_d)
                real_samples_loss = loss_func(real_samples_pred, torch.ones_like(real_samples_pred))
                fake_samples_pred = model['d'](fake_samples.detach(), onehot_fake_labels_d)
                fake_samples_loss = loss_func(fake_samples_pred, torch.zeros_like(fake_samples_pred))
                loss: torch.Tensor = 0.5 * (real_samples_loss + fake_samples_loss)
                ...
                fake_labels_pred = model['d'](fake_samples, onehot_fake_labels_d)
                loss: torch.Tensor = loss_func(fake_labels_pred, torch.ones_like(fake_labels_pred))
